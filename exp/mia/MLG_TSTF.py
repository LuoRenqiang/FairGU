...
DATASET_ALIAS = {
    ...
    "physics": "Physics",
    "phys": "Physics",
    "coauthor_phys": "Physics",
    "flickr": "flickr",
}

DATASET_SPLIT_CONFIG = {
    "cora": (90, 90, 630, 630),
    "citeseer": (100, 100, 600, 600),
    "pubmed": (1500, 1500, 4500, 4500),
    "cs": (10, 400, 150, 6000),
    "physics": (1000, 1000, 3000, 3000),
    "flickr": (1500, 1500, 10500, 10500),
    "credit": (1000, 1000, 3000, 3000),
    "pokec_z": (1000, 1000, 3000, 3000),
    "pokec_n": (1000, 1000, 3000, 3000),
    "income": (800, 800, 2400, 2400),
}
...
class MIA:
    def __init__(
        ...
        for which_run in range(1, num_of_runs):
            ...
            os.makedirs("exp/mia/pro_data", exist_ok=True)
            os.makedirs("exp/mia/result", exist_ok=True)

            save_shadow_OutTrain = (
                "exp/mia/pro_data/posteriorsShadowOut_"
            ...
            save_shadow_InTrain = (
                "exp/mia/pro_data/posteriorsShadowTrain_"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            save_target_OutTrain = (
                "exp/mia/pro_data/posteriorsTargetOut_"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            save_target_InTrain = (
                "exp/mia/pro_data/posteriorsTargetTrain_"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            save_correct_incorrect_homophily_prediction = (
                "exp/mia/pro_data/correct_incorrect_homo_pred"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            save_target_InTrain_nodes_neigbors = (
                "exp/mia/pro_data/nodesNeigborsTargetTrain_"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            save_target_OutTrain_nodes_neigbors = (
                "exp/mia/pro_data/nodesNeigborsTargetOut_"
                + str(self.args["unlearn_ratio"])
                + "_"
                + mode
                + "_"
            ...
            ######################################## Data ##############################################
            """
            dataset_lookup = data_type.lower()
            if dataset_lookup in DATASET_SPLIT_CONFIG:
                (
                    num_train_Train_per_class,
                    num_train_Shadow_per_class,
                    num_test_Target,
                    num_test_Shadow,
                ) = DATASET_SPLIT_CONFIG[dataset_lookup]
                dataset_name = self._normalize_dataset_name(data_type)
                dataset = self._load_dataset_from_store(dataset_name)
                ...
                raise ValueError(f"Unsupported dataset for MIA: {data_type}")
            ...
            if isinstance(self.node_removes, np.ndarray):
                self.node_removes = torch.from_numpy(self.node_removes)
            elif not isinstance(self.node_removes, torch.Tensor):
                self.node_removes = torch.tensor(self.node_removes)
            self.node_removes = self.node_removes.long()
            ...
            num_target_classes = self.train_posterior.shape[1]
            num_shadow_classes = positive_attack_data.shape[1]
            num_features = max(num_target_classes, num_shadow_classes)
            target_columns = [str(i) for i in range(num_features)]
            
            train_posterior_np = self.train_posterior.cpu().detach().numpy()
            if train_posterior_np.shape[1] < num_features:
                if num_features == 2 and train_posterior_np.shape[1] == 1:
                    expanded = np.zeros((train_posterior_np.shape[0], 2))
                    expanded[:, 0] = train_posterior_np[:, 0]
                    expanded[:, 1] = 1 - train_posterior_np[:, 0]
                    train_posterior_np = expanded
                else:
                    expanded = np.zeros((train_posterior_np.shape[0], num_features))
                    expanded[:, :train_posterior_np.shape[1]] = train_posterior_np
                    train_posterior_np = expanded
            ...
            test_posterior_np = self.test_posterior.cpu().detach().numpy()
            if test_posterior_np.shape[1] < num_features:
                if num_features == 2 and test_posterior_np.shape[1] == 1:
                    expanded = np.zeros((test_posterior_np.shape[0], 2))
                    expanded[:, 0] = test_posterior_np[:, 0]
                    expanded[:, 1] = 1 - test_posterior_np[:, 0]
                    test_posterior_np = expanded
                else:
                    expanded = np.zeros((test_posterior_np.shape[0], num_features))
                    expanded[:, :test_posterior_np.shape[1]] = test_posterior_np
                    test_posterior_np = expanded
            ...
            sample_features, _ = next(iter(attack_train_data_loader))
            input_dim = sample_features.shape[1]  # Get actual feature dimension
    ...
    def _normalize_dataset_name(self, dataset_name: str):
        if not isinstance(dataset_name, str):
            return dataset_name
        return DATASET_ALIAS.get(dataset_name.lower(), dataset_name)

    def _load_dataset_from_store(self, dataset_name: str):
        normalized_name = self._normalize_dataset_name(dataset_name)
        store_args = dict(self.args)
        store_args["dataset_name"] = normalized_name
        data_store = DataStore(store_args)
        data = data_store.load_raw_data()
        return self._post_process_dataset(normalized_name, data)

    def _post_process_dataset(self, dataset_name: str, data):
        if dataset_name == "Physics":
            feature = data.x
            n_components = min(500, feature.shape[1])
